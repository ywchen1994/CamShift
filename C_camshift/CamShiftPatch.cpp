...
IplImage* CamShiftPatch::drawHistImg(CvRect selectROI, CvScalar maskRange)
{
	...
	cvShowImage("originImage", originImage);
	//---設定ROI和畫出直方圖---
	...
	hist = cvCreateHist(1, &hdims, CV_HIST_ARRAY, &hranges, 1); //設定直方圖的格式
	cvCalcHist(&hue, hist, 0, mask); //以hue資訊 得到直方圖 (只有ROI部分的資訊)
	cvGetMinMaxHistValue(hist, 0, &max_val, 0, 0); //只找最大值 
	cvConvertScale(hist->bins, hist->bins, max_val ? 255. / max_val : 0., 0); //縮放bin到區間[0,255]  
	...
	{
		int val = cvRound(cvGetReal1D(hist->bins, i)*histimg->height / 255);  //cvGetReal1D(直方條高度，第幾條) 
...
CvBox2D CamShiftPatch::getTrackBox(CvScalar maskRange)
{
